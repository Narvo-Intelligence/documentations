...
- **Subscription state (current):**
  - Stored in Supabase table `user_subscriptions` (one row per user: `user_id`, `tier`, `status`, `started_at`, `ends_at`, `external_id`).
  - Backend API: `GET/PUT/DELETE /api/subscription/{user_id}` and `POST /api/subscription/{user_id}/cancel` for CRUD and cancel. See `backend/services/subscription_service.py`.
  - Frontend: **Subscription** page (`/subscription`) lets users view current plan (Free / Pro / Enterprise), upgrade, downgrade, or cancel. Data is read/updated in real time via the API.
- **Current** features require no tier checks today; **Proposed** and **Recommended** items need:
  - Backend enforcement for limits (TTS, fact-check, briefing count, offline storage) using `tier` from `user_subscriptions` or settings.
  - Frontend gating or upsell for Pro/Enterprise-only features.
- Payment and subscription lifecycle (Stripe, etc.) are out of scope of this doc but required for Pro and Enterprise. The `external_id` column can store a payment provider subscription ID when integrated.

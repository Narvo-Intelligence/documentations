...
| **Geolocation** (optional, with consent) | Get approximate location. | Optional “Local news near you” or region prefill; can also power a lightweight **Dashboard weather** tile (fallback to a default city if permission denied). |
...
| **Geolocation** | **Onboarding** or **Settings** (or **Dashboard** for weather). | Optional “Use my location for local news”; pass region to `/api/news` or search; can also fetch weather for the sidebar widget (fallback when denied). |
